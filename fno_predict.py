...
def main():
    ...
    if train_cfg:
        print("[model] trained with " + ", ".join(
            f"{k}={train_cfg[k]}" for k in
            ("CHANNELS", "MODES", "LAYERS", "EPOCHS", "BATCH_SIZE",
             "STEPS_PER_EPOCH", "MAX_UNROLL", "BPTT", "AMP", "ZERO_DC")
            if k in train_cfg))
    else:
        print("[model] no train_config in the checkpoint (written by an "
              "older fno_train.py); results will not be traceable to settings")
    ...
    with open(os.path.join(RESULTS_DIR, f"{LABEL}_summary.json"), "w") as fh:
        json.dump({"model_path": MODEL_PATH,
                   "model_config": model.config,
                   "train_config": train_cfg,
                   "selected": ckpt.get("best"),
                   "eval": {"lattice": LATTICE, "off": OFF,
                            "t_start": START_TIME, "t_end": END_TIME,
                            "threshold": R2_THRESHOLD,
                            "early_stop": EARLY_STOP},
                   "seeds": summary}, fh, indent=1)
