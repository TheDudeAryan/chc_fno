...
# -- rollout training ------------------------------------------------------
# MAX_UNROLL was 16 on the first A100 run and that stage was purely harmful:
# held-out one-step loss went 0.00620 -> 0.00721 and mean rollout survival
# 1719 -> 1570 once K reached 16.  Since accumulated error grows roughly
# linearly in t, per-step accuracy is what sets survival, so trading it for
# extra long-horizon stability is a losing trade.  See README section 5a.
MAX_UNROLL = 8
BPTT = 2                       # trailing steps that carry gradient
WARM_FRAC = 0.15               # fraction of epochs at unroll = 1
RAMP_FRAC = 0.55               # reach MAX_UNROLL by here, then hold
NOISE_STD = 0.01
NOISE_PROB = 0.5
...
CONFIG_KEYS = (
    "DATASET", "CHANNELS", "MODES", "LAYERS", "MODE_SCALING", "ZERO_DC",
    "LOCAL_KERNEL", "EPOCHS", "BATCH_SIZE", "STEPS_PER_EPOCH", "LR",
    "WEIGHT_DECAY", "GRAD_CLIP", "LR_FLOOR", "MAX_UNROLL", "BPTT", "WARM_FRAC",
    "NOISE_STD", "NOISE_PROB", "H1_WEIGHT", "SMOOTH_WEIGHT", "VAL_SEEDS",
    "VAL_T_END", "VAL_EVERY", "AMP", "TF32", "COMPILE", "GPU_DATA", "SEED")


def config_snapshot():
    """The CONFIG block as a dict, recorded into every checkpoint.

    Without this a result file cannot be traced back to the settings that
    produced it -- which matters as soon as runs from more than one machine
    are being compared.
    """
    g = globals()
    return {k: g[k] for k in CONFIG_KEYS if k in g}


def save_model(model, path, **extra):
    torch.save({"config": model.config, "state_dict": model.state_dict(),
                "train_config": config_snapshot(), **extra}, path)
...
def rollout_score(model, gt_times, gt_frames, device, amp_ctx=None,
    ...
    """Mean of max(R2, 0) over the fixed window, and the time R2 first drops
    below ``threshold``.  Returns (score, survival).

    Selection uses the score, not the survival time.  Survival is a threshold
    crossing, and on the A100 run it swung by ~300 time units between adjacent
    validations, so picking the best of ~40 of them is largely a lottery.  A
    mean over a *fixed* window is far smoother.  The window must not depend on
    where the rollout failed -- a mean over an early-stopped rollout flatters
    short rollouts, which stop before the error grows -- so the full horizon is
    always run.

    R2 is floored at 0 before averaging.  Below zero the prediction is worse
    than the field's own mean, and one diverged rollout reaching R2 = -300
    would otherwise swamp the average and rank models by how badly they blow
    up rather than by how long they stay right.  Floored, the score is bounded
    in [0, 1] and reads as "the fraction of the requested rollout the model
    got right".
    """
...
def unroll_at(epoch, epochs, max_unroll, warm_frac=WARM_FRAC,
    ...
    Spreading the stages over the whole run (the first version) puts the
    hardest objective in the final epochs, where the cosine schedule has
    already decayed the learning rate to a few percent of peak -- the model
    meets the demanding horizon exactly when it can no longer adapt to it.  On
    the A100 run K = 16 began at epoch 161 with lr = 2.0e-4 and never beat the
    K = 8 stage that preceded it.  Reaching the maximum early and holding it
    leaves the low-LR epochs to polish the objective that will actually be
    used at test time.
    """
    if max_unroll <= 1:
        return 1
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for _ in range(STEPS_PER_EPOCH):
            ...
            # Mixed horizon, sampled over powers of two rather than uniformly
            # over 1..k_max.  Uniform sampling gives the one-step map only
            # 1/k_max of the batches, so raising the ceiling silently starves
            # exactly the thing that sets rollout length; over {1,2,...,k_max}
            # its share is 1/(log2(k_max)+1) and stays put as the ceiling rises.
            k = int(2 ** rng.integers(0, int(math.log2(k_max)) + 1))
            x = win[:, 0:1]
            ...
            if n_detached:
                with torch.no_grad(), amp_ctx():
                    for _ in range(n_detached):
                        x = step_model(x)
                # A diverged detached rollout poisons the batch: the A100 run
                # logged a single epoch at train = 1.02 against ~0.014
                # neighbours.  Gradient clipping bounds the step but the batch
                # still contributes nothing useful, so drop it.
        ...
        rec = {"epoch": epoch + 1, "unroll": k_max, "train": train_loss,
               "val1": val_loss, "seconds": secs, "skipped": skipped}

        if (epoch + 1) % VAL_EVERY == 0 or epoch == EPOCHS - 1:
            per_seed = [rollout_score(model, t, f, str(device), amp_ctx)
                        for t, f in gts]
            mean_r2 = float(np.mean([p[0] for p in per_seed]))
            surv = float(np.mean([p[1] for p in per_seed]))
            rec.update(mean_r2=mean_r2, survival=surv,
                       per_seed=[p[1] for p in per_seed])
            line += (f" | rollout R2 {mean_r2:.4f} | survives t={surv:6.0f} "
                     f"{[int(p[1]) for p in per_seed]}")
            # selection on the mean, not the threshold crossing -- see
            # rollout_score for why
            if mean_r2 > best["mean_r2"]:
                best = {"mean_r2": mean_r2, "survival": surv,
                        "epoch": epoch + 1,
                        "per_seed": [p[1] for p in per_seed]}
        ...
        torch.save({"epoch": epoch + 1, "state_dict": model.state_dict(),
                    ...
                    "config": model.config, "history": history,
                    "train_config": config_snapshot()}, CHECKPOINT)
    ...
    print(f"[done] best rollout R2 = {best['mean_r2']:.4f} "
          f"(survives to t = {best['survival']:.0f}, epoch {best.get('epoch')})"
          f"; model saved to {MODEL_OUT}", flush=True)
